...
/**
 * Marketing · Content Agent — V2 take on the Manus ContentAgent page.
 * The Manus version calls external AI services (hooks/scripts/captions via
...
export function ContentAgentView() {
  ...
  const { data: briefs, isLoading } = useQuery({
    queryKey: ["marketing", "briefs", "content"],
    queryFn: () => dal.marketing.briefs("content"),
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ["marketing", "briefs", "content"] });

  const saveMut = useMutation({
    mutationFn: (b: { id: string; title: string; brief: string; status: BriefStatus }) =>
      withSync(dal.marketing.upsertBrief({ ...b, kind: "content" }, actor)),
  ...
  return (
    <div className="mx-auto max-w-4xl pt-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black uppercase text-zinc-100">Content Agent</h1>
      ...
      <p role="note" className="mt-4 rounded-xl border border-amber-700/50 bg-amber-950/40 px-4 py-3 text-sm text-amber-300">
        ...
        queue tracks the real content brief workflow — nothing here is machine-generated.
      </p>
        ...
        <ul className="mt-4 space-y-3">
          ...
          {list.length === 0 && (
            <li className="rounded-xl border border-dashed border-ink-700 py-12 text-center text-sm text-zinc-500">
              No content briefs yet — add the first one.
            </li>
          )}
        </ul>
      )}
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="New brief"
      ...
      <form className="w-full max-w-md rounded-2xl border border-ink-700 bg-ink-900 p-5"
        ...
        <h3 className="text-lg font-bold text-zinc-100">New content brief</h3>
